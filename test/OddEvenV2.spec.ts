  ...
  it("tst-item-001-usr1-play", async () => {
    ...
    try {
      ...
      log.debug(`S2-Ent usr1 bal :: ${ethers.utils.formatUnits( await usr1.getBalance(), "ether")}`)
        ...
        let transamount = ethers.utils.parseUnits( "10", 18 )

        let playCalldata = oddeveninst.interface.encodeFunctionData(
                                        'play',
                                        [2]
                                      )
        ...
        log.debug(`======================== Usr2 Trans ========================`)

        log.debug(`S2-Ent usr2 bal :: ${ethers.utils.formatUnits( await usr2.getBalance(), "ether")}`)

        transamount = ethers.utils.parseUnits( "10", 18 )

        playCalldata = oddeveninst.interface.encodeFunctionData(
                                        'play',
                                        [ 10 ]
                                    )

        trancnt = await usr2.provider.getTransactionCount(await usr2.getAddress())
